  ...
  try {
    ...
    if (Object.keys(updates).length) {
      ...
      if (parsed.data.lines && parsed.data.lines.length) {
        ...
        await supabase.from("po_line_items").delete().eq("po_id", params.id).eq("company_id", guard.employee.company_id);
        const { data: updatedLines, error: linesError } = await supabase.from("po_line_items").insert(lineItems).select();
        if (linesError) return NextResponse.json({ error: linesError.message }, { status: 500 });
        return NextResponse.json({ po, lineItems: updatedLines ?? [] });
      }
...
export async function DELETE(_request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (error) {
      if (error.code === "PGRST116") return NextResponse.json({ error: "Purchase order not found" }, { status: 404 });
