...
export async function GET(_: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!canViewExpenseClaim(visibility, claim)) return NextResponse.json({ error: "You do not have permission to view this expense claim." }, { status: 403 });
    ...
    if (visibility.canApprove && !pendingStep && claim.status === "submitted" && employee?.id && claim.employees?.reporting_manager_id === employee.id && effectiveToggles(template?.toggles, employee?.permission_overrides).approve_expenses === true) {
      const { data: repairedStep } = await approvalDb.from("approval_steps").insert({ entity_type: "expense_claim", entity_id: claim.id, company_id: claim.company_id, level: 1, approver_employee_id: employee.id, status: "pending" }).select("id, level, status, comment, decided_at").single();
    ...
    return NextResponse.json({ claim: { ...claim, expense_line_items: lineItems }, approvalStep, can_approve: visibility.canApprove, can_pay: visibility.canPay || guard.profile.role === "company_admin" });
  } catch (error) { return error as Response; }
}

export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  try {
    const guard = await requireFinance("finance_expense_claims");
